...
import { useGetShiftStatusQuery, useCloseCashRegisterMutation } from 'src/redux/services/posApi';
            ...
            <Stack direction={{ xs: 'column', md: 'row' }} spacing={4}>
              <Box sx={{ flex: 1 }}>
                ...
                <Stack spacing={0.5}>
                  ...
                  <Row label="Hora de apertura" value={fDateTime(shiftStatus.opened_at, 'dd MMM yyyy HH:mm')} />
                ...
                {Object.keys(shiftStatus.payment_methods_breakdown).length > 0 && (
                  <>
                    ...
                    <Stack spacing={0.25} sx={{ mt: 0.5 }}>
                      {Object.entries(shiftStatus.payment_methods_breakdown).map(
                        ([method, amount]: [string, string]) => (
                          <Row key={method} label={PAYMENT_LABEL[method] || method} value={fCurrency(Number(amount))} />
                        )
                      )}
                    </Stack>
                  </>
